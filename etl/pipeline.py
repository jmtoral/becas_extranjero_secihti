...
def build_analysis_tables(
    ...
    institution = (
        standardized.groupby("institution_canonical", dropna=False)
        .agg(
            scholarship_count=("snapshot_id", "count"),
            amount_real_mxn_2020=("amount_real_mxn_2020", lambda series: series.sum(min_count=1)),
        )
    ...
    institution_year = (
        standardized.groupby(["source_year", "institution_canonical"], dropna=False)
        .agg(
            scholarship_count=("snapshot_id", "count"),
            amount_nominal_mxn=("amount_nominal_mxn", lambda series: series.sum(min_count=1)),
            amount_real_mxn_2020=("amount_real_mxn_2020", lambda series: series.sum(min_count=1)),
...
def write_site_seed(
    ...
    json_output = SITE_DATA_DIR / "summary.json"
    js_output = SITE_DATA_DIR / "summary.js"
    serialized = json.dumps(payload, ensure_ascii=False, indent=2)
    json_output.write_text(serialized, encoding="utf-8")
    js_output.write_text(f"window.__SITE_SUMMARY__ = {serialized};\n", encoding="utf-8")
